...
#                 mapped_value = self.config.recurrence_mapping[value]
                self.recurrence_bool[id] = value
                self.recurrence_value[id] = curr_patient['Days']

#                 end_date = curr_patient['Date of Last Known Alive'].split('/')
#                 date_end_date = datetime.date(int(end_date[2]),
#                                                    int(end_date[0]),
#                                                    int(end_date[1]))
#                 date_of_ct = curr_patient['CT Date'].split('/')
#                 date_of_ct = datetime.date(int(date_of_ct[2]),
#                                            int(date_of_ct[0]),
#                                            int(date_of_ct[1]))
#                 days_under_observation = (date_end_date - date_of_ct).days
#                 self.durations[id] = days_under_observation

#                 if mapped_value == 1:
#                     date_of_recurrence = curr_patient['Date of Recurrence'].split('/')
#                     date_of_recurrence = datetime.date(int(date_of_recurrence[2]),
#                                                        int(date_of_recurrence[0]),
#                                                        int(date_of_recurrence[1]))
#                     date_of_ct = curr_patient['CT Date'].split('/')
#                     date_of_ct = datetime.date(int(date_of_ct[2]),
#                                                int(date_of_ct[0]),
#                                                int(date_of_ct[1]))
#                     days_between_ct_surgery = int(curr_patient['Days between CT and surgery'])
#                     days_to_recurrence = (date_of_recurrence - date_of_ct).days - days_between_ct_surgery
#                     self.recurrence_value[id] = days_to_recurrence
#                 else:
#                     self.recurrence_value[id] = 'N/A'
#             else:
#                 self.recurrence_bool[id] = 'N/A'
#                 self.recurrence_value[id] = 'N/A'
        return
